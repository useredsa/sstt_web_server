...
int send_internal_server_error(bool keepAlive = false) {
    return send_static_file(STATUS_INTERNAL_SERVER_ERROR,
      "data/internal_server_error.html", keepAlive, true);
}

int send_unsupported_media_type(bool keepAlive = true) {
    return send_static_file(STATUS_UNSUPPORTED_MEDIA_TYPE,
      "data/unsupported_media_type.html", keepAlive, true);
}

int send_request_entity_too_large(bool keepAlive = false) {
    return send_static_file(STATUS_REQUEST_ENTITY_TOO_LARGE,
      "data/request_entity_too_large.html", keepAlive, true);
}

int send_method_not_allowed(bool keepAlive = false) {
    return send_static_file(STATUS_METHOD_NOT_ALLOWED,
      "data/method_not_allowed.html", keepAlive, true);
}

int send_version_not_supported(bool keepAlive = false) {
    return send_static_file(STATUS_VERSION_NOT_SUPPORTED,
      "data/version_not_supported.html", keepAlive, true);
}

int send_not_implemented(bool keepAlive = false) {
    return send_static_file(STATUS_NOT_IMPLEMENTED,
      "data/version_not_supported.html", keepAlive, true);
}

/**
 * @brief Sends a simple message consisting of a status line.
 * 
 * Useful for sending an error code after detecting an internal
 * error.
 */
...
/**
 * @brief Writes an HTTP response to the client_socket.
 * 
 * The function serves the purpose of returning a file.
 * You must specify an HTTP response code, a path and whether
 * the connection must be kept open.
 * 
 * If the pathname is not valid, the file does not exists, or
 * another error is found, an appropiate error message is sent
 * instead.
 * 
 * Additionally, you can specify if this file is an internal file.
 * In that case, if an error occurs only the status line will be
 * sent, in order not so change the meaning of the message (the
 * error that occurs first should be sent).
 */
int send_static_file(int code, const char* pathname, bool keepAlive, bool isInternalFile) {
    // Gather of information and error check
