...
\begin{itemize}
\item La captura \key{capture-home-page.pcapng} muestra los mensajes intercambiados para cargar la página de inicio.

\item La captura \key{success-persistent.pcapng} contiene además los mensajes tras introducir el correo electrónico en el formulario. Puede servir para verificar la persistencia, viendo como el servidor mantiene la conexión abierta unos segundos esperando a la segunada acción.

\item La captura \key{capture-not-found.pcapng} muestra una respuesta \key{404 Not Found}.

\item La captura \key{capture-unsupported-media-type.pcapng} muestra una respuesta \key{415 Unsupported Media Type}.

\item La captura \key{capture-request-entity-too-large.pcapng} nos muestra el comportamiento del servidor ante un mensaje {\POST} de \SI{9}{\kilo\byte}. Aunque como hemos explicado antes el servidor está preparado para procesar mensajes de tamaño arbitrario leyéndolos en bloques de \SI{8}{\kibi\byte}, hemos establecido el máximo en \SI{8}{\kibi\byte}. En la captura podemos ver que el servidor devuelve el mensaje de error \key{413 Request Entity Too Large} y corta la conexión.

\item La captura \key{capture-pipelining.pcapng} nos muestra la respuesta del servidor ante un cliente que lleva a cabo \key{pipelining} con un ejemplo en el que llegan $4$ mensajes {\GET} a la vez.
